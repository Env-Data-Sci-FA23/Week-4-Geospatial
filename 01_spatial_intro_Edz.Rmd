...
# Set up script I just pasted this into this code chunk in place of running the source=setup.R from the week four folder - same same but clunkier I think
...
terra:: extract(co_elevation,poudre_points_prj)
    ```
Here are the elevations for the three points from 'poudre_points_prj'
...
```{r}
mi_elevation <- get_elev_raster(mi_counties, z = 7)
...
```{r}
mi_elevation <- rast(mi_elevation)
```
...
```
```{r}
mi_elevation_crop<- crop(mi_elevation, mi_counties, mask=TRUE)
```

```{r}
qtm(mi_elevation_crop)
```
...
```{r}
kazoo<-filter(mi_counties,NAME == "Kalamazoo")

kazoo_elev<-crop(mi_elevation,kazoo,mask=TRUE)

qtm(kazoo_elev)
```

This worked and generated an elevation heat map between 150 and 400 ft asl for a square county (Kalamazoo) The only notable feature is the kalamazoo river basin which is below the rest of the county. 
