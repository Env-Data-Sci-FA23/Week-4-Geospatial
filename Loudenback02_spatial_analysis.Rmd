...
```{r}
occ_sp_prj<- st_transform(occ_sp, st_crs(rivers))
st_crs(occ_sp_prj) == st_crs(rivers)
```
...
```{r}
qtm(rivers) +
  qtm(occ_sp_prj, symbols.col = "Species")
```
...
Similar to `filter()` from the {tidyverse}, we can use `st_filter()` to perform a *spatial* filtering (i.e., we want to filter just the points that occur in Larimer County).
...
```{r}
Larimer_county <- counties[counties$NAME == "Larimer", ]

occ_larimer <- st_filter(occ_sp_prj, Larimer_county)
...
occ_larimer2 <- occ_larimer %>%
  mutate(river_dist_m = as.numeric(river_dist_m))
  average_distance <- occ_larimer %>% 
    group_by(Species) %>%
    summarise(River_meters=mean(river_dist_m))

ggplot(average_distance, aes(x=Species, y=River_meters)) +
  geom_bar(stat = "identity") +
  labs(title = "Average distance from rivers in Larimer County by species",
       x = "Species",
       y = "Distance") +
  theme_minimal()
...
Which species is, on average, found closest to a river? Elk are found closest to the river. 
...
We would want to be cautious about using these results to answer real research questions because we don't know how accurate the spatial data associated with the political boundaries is. Also, we would want to be sure that the CRS matches between our data and the political boundary. 
...
landcover_prj <- project(landcover, crs(occ_sp_prj))

terra::extract(landcover_prj, occ_sp_prj)
```
...
```{r}
occ_landcover <- occ_sp_prj %>%
  mutate(common_landcover = extract(landcover_prj, occ_sp_prj)) %>%
...
```{r}
# Extract landcover values within Colorado counties
county_landcover <- 
  counties %>%
  mutate(landcover = extract(landcover_prj, counties, fun = "modal")) %>%
  unnest(landcover) %>%
  rename(value = "NLCD Land Cover Class")

# Get a table of value - class pairs
classes <- as.data.frame(cats(landcover))

# Join county_landcover and classes data frames
merged_data <- left_join(county_landcover, classes, by = c("value" = "value"))

fill_colors <- c("Urban" = "red", "Agriculture" = "green", "Forest" = "blue")

# Create a map of the counties colored/filled by the most common NLCD land cover class
ggplot(merged_data, aes(fill = NLCD.Land.Cover.Class)) +
  geom_sf() +
  scale_fill_manual(values = fill_colors) +
  labs(title = "Counties Colored by Most Common NLCD Land Cover Class") +
  theme_classic()
```
...
# Transform occurrences object to match the CRS of the elevation raster
occ_prj <- st_transform(occ_sp_prj, st_crs(elevation))

# Extract elevation values for each occurrence
occ_prj$elevation <- terra::extract(elevation, occ_prj)

# Calculate the average elevation for each species
average_elevation_by_species <- occ_prj %>%
  group_by(Species) %>%
  summarise(average_elevation = mean(elevation, na.rm = TRUE))

# Identify the species with the highest average elevation
species_highest_elevation <- average_elevation_by_species %>%
  arrange(desc(average_elevation)) %>%
  head(1)

# Print the result
print(species_highest_elevation)
