...
occ_larimer_avg <- occ_larimer %>% 
  group_by(Species) %>%
  summarise(avg_river_dist = mean(river_dist_m)) %>%
   mutate(avg_river_dist = as.numeric(avg_river_dist))
```

```{r}
ggplot(occ_larimer_avg) +
  geom_col(aes(x= Species, y = avg_river_dist, fill = Species))
```

**Which species is, on average, found closest to a river?**

Elk on average are found closest to a river.
...
landcover_freq <- freq(landcover) %>%
  rename(landcover = value, frequency = count) %>%
  select(-layer)
```

```{r}
ggplot(landcover_freq) +
  geom_col(aes(x = reorder(landcover, -frequency), y = frequency)) +
  labs(x = "landcover") + 
  coord_flip()
```
...
The yellow-bellied marmot is on average found at the highest elevation.
