...
```{r}
co_terra<- terra::extract(co_elevation,poudre_points_prj)
```
...
ut_roads <- roads(state = "UT", county = "Iron")
```
...
tm_shape(ut_elevation) +
  tm_raster(style = "cont",title = "Utah's Elevation (m)")
```

```{r}
ut_elevation1 <- rast(ut_elevation)
```
...
```{r}
utah_elevation_crop <- crop(ut_elevation1,IC,mask=T)
```

```{r}
qtm(utah_elevation_crop)+
  qtm(IC,fill=NULL,title = "Cedar City")
```

```{r}
tm_shape(utah_elevation_crop) +
  tm_raster(style = "cont",title = "Utah's Elevation (m)")
```
