...
occ_larimer <- occ_larimer %>% 
  group_by(Species) %>%
  mutate(species_avg = as.numeric(mean (river_dist_m)))


ggplot(occ_larimer, aes(x = Species, y = species_avg)) +
...
The Western Tiger Salamander is found closest to a river on average. 
...
Political boundaries are not an indication of ecological boundaries, and we could be excluding data that is relevant to this analysis. 
...
```{r}
landc_freq <- freq(landcover)

ggplot(landc_freq, aes(x = reorder(value, -count), y = count)) + geom_bar(stat = "identity") + coord_flip()
...
county_classes <- inner_join(county_landcover, classes, by = "value")

qtm(county_classes, fill = "NLCD.Land.Cover.Class")
...
# terra::extract(co_elevation_crop, poudre_points_prj)

sp_elevation <- terra::extract(elevation, occ_prj)


occ_prj$elevation <- sp_elevation 

elev_avg <- occ_prj %>%
  drop_na() %>%
  group_by(Species) %>%
  summarise(mean_value = mean (elevation$Elevation))

```
Yellow-bellied marmots on average are found at the highest elevation. 
