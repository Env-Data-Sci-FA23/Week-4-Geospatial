...
```{r}
occ_prj <- st_transform(occ_sp, crs = 4326)

st_crs(occ_prj)

river_prj <- st_transform(rivers, crs = 4326)
st_crs(river_prj)
```
...
larimerc <- counties %>%
  filter(NAME == "Larimer") %>%
  st_as_sf(coords = c("INTPTLON", "INTPTLAT"), crs = 4326)

larimerc_prj <- st_transform(larimerc, crs = 4326)

st_crs(larimerc_prj) == st_crs(occ_prj)

occ_larimer <- st_filter(occ_prj, larimerc_prj)
```
...
```{r}
occ_larimer$nearest_river <- st_nearest_feature(occ_larimer, river_prj)
```
...
occ_larimer$river_dist_m <-
  st_distance(occ_larimer, river_prj[occ_larimer$nearest_river, ], by_element = TRUE)
```
...
```{r}
# need to make a new numeric column
occ_larimer <- occ_larimer %>%
  mutate(riverdist = as.numeric(occ_larimer$river_dist_m))

# calculate average distance for each species:
occ_larimer_avg <- occ_larimer %>%
  group_by(Species) %>% 
  summarize(avgdist = mean(riverdist))

# making the barplot
ggplot(occ_larimer_avg, mapping = aes(Species, avgdist, fill = Species)) +
  geom_bar(stat = "identity")
```

Which species is, on average, found closest to a river?

The species that is found closest to a river on average is Elk.
...
```{r eval=FALSE}
river_buffer <- st_buffer(river_prj, dist = 100)
...
# lengths with an s - treats each row as value instead of as a list - bc if we did length with no s then it would give us 1 for the length of the list itself that is populating the row of this column*
```
...
```{r}
# need to make landcover into a dataframe in order to plot
landcover_df <- freq(landcover)

ggplot(landcover_df, aes(x = reorder(value, -count), y= count, fill = value))+
  geom_bar(stat = "identity") +
  xlab("Landcover")
```
...
```{r}
# joining the df:
county_landcover_j <- county_landcover %>%
  left_join(classes, by = "value")
  
# making a map of counties filled by most common land cover class:
qtm(county_landcover_j, fill = "NLCD.Land.Cover.Class")
...
```{r}
# extract elevation for each point of species and then calculate average

# check if elevation and occ_prj are in the same CRS
st_crs(elevation) == st_crs(occ_prj) # FALSE, so projecting elevation

elevation_prj <- project(elevation, "EPSG:4326")
st_crs(elevation_prj) == st_crs(occ_prj) # TRUE!
...
# finding average elevation for each species:
elev_occ_avg <- elev_occ %>%
  st_drop_geometry() %>%
  group_by(Species) %>% 
  summarize(avgelev = mean(Elevation, na.rm = TRUE))

# making a barplot to visualize the results:
ggplot(elev_occ_avg, aes(x = reorder(Species, -avgelev), y= avgelev, fill = Species))+
  geom_bar(stat = "identity") +
  xlab("Species") +
  ylab("Average Elevation")

# The Yellow-Bellied Marmot is, on average, found at the highest elevations.
```
