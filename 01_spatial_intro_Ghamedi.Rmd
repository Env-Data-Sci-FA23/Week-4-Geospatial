...
Lets view our Colorado counties and Larimer County roads shapefiles. To make a "quick thematic map" in `tmap` you can use the `qtm()` function. You can also use `tm_shape()` plus the type of spatial layer (e.g., `tm_polygons()`) to add your layers to the map. Both methods below will produce the same exact map, and you may think why would you ever need to use the `tm_shape()` method since it's more code? The answer may be rarely, but there are some cases where you can customize your maps better with `tm_shape()` that we will see later on.
...
# if I want to crop to a bounding polygon
# co_elevation_crop <- crop(co_elevation, poudre_hwy, mask = TRUE)
...
1.  **Explore the use of `extract` from the `terra` package by running `?terra::extract`. (Note we need to specify `terra::` because 'extract' is a function name in multiple packages we may have loaded in our session).**
    ...
    **How would you extract the elevation at each of the three points in `poudre_points_prj` ? (2 pts)**
    ...
    poudre_point_elv <- terra::extract(co_elevation, poudre_points_prj)
...
```{r}
va_elevation <- get_elev_raster(va_counties, z = 7)

qtm(va_elevation)

class(va_elevation)

va_elevation <- rast(va_elevation)

va_elevation_crop <- crop(va_elevation, va_counties , mask = TRUE)

qtm(va_elevation_crop)
```
...
```{r}
# Looking at Montgomery County, VA 

mont_county <- va_counties %>% 
  filter(NAME == "Montgomery")

mont_crop <- crop(va_elevation, mont_county, mask = TRUE)
