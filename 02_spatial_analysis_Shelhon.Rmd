d--- title: "Spatial Data Analysis" author: "Caitlin Mothes" date: "`r Sys.Date()`" output: github_document ---
...
```{r}
joined_df <- full_join(county_landcover, classes, by = c("value" = "NLCD.Land.Cover.Class") %>%
  st_drop_geometry() %>% 
  group_by(NLCD.Land.Cover) %>% 
  count(common_landcover) %>% 
  slice(which.max(n))
