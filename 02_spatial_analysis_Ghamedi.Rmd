...
```{r}
# transform the CRS of occ_sp to the CRS of rivers
...
filt_sp <- occ_larimer %>%
  ...
  summarize(Avg_dist = mean(river_dist_m))

str(filt_sp)

filt_sp_fin <- filt_sp %>% mutate_at('Avg_dist', as.numeric)
  
str(filt_sp_fin) 

ggplot(filt_sp_fin, aes(x = as.factor(year), y = Avg_dist, fill = Species)) +
        geom_bar(stat = "identity", position = "dodge")

ggplot(filt_sp_fin, aes(x = Species, fill = year, y = Avg_dist)) +
...
Based on my questionable plot -- Elk are closest

## Buffers

Alternatively, say you want to know what percentage of species' occurrences (points) were found within a specified distance of a river (calculated buffer). Here let's investigate how often each species is found within 100m of a river.
...
Now we can calculate what percentage of occurrences are within 100 m of a river for each species using `dplyr` operations. Which species is most often found within 100m of a river? Here, Elk are found most often within 100 m of a river
...
Create a bar chart of landcover frequency, and order the bars highest to lowest (see [this resource](https://sebastiansauer.github.io/ordering-bars/) to guide you on sorting bars by a numeric variable/column). Also investigate the use of `coord_flip()` and how it might make your plot look better...

```{r}
library(scales)

#This plot is high to low (-count); removing the "-" makes it high to low
#ggplot(freq(landcover), aes(x = reorder(value, -count), y = count)) + geom_bar(stat = "identity") + 
# coord_flip()
...
```{r}
join_cover <- left_join(county_landcover, classes, by = "value")
...
# project the elevation layer
elevation_prj <- project(elevation, crs(occ_prj))

extract(elevation_prj, occ_prj)

occ_elevation <- occ_prj %>%
  mutate(species_elv = extract(elevation_prj, occ_prj)) %>%
  unnest(species_elv) %>% 
  #lets rename the species column which is now called "Species Class"
  rename(species_elv = "Elevation")

occ_elevation %>% 
  st_drop_geometry() %>% # this converts the data back to a dataframe, required for some tidyverse operations
  group_by(Species) %>% 
  count(species_elv) %>% 
  slice(which.max(n)) #returns the row with the highest count "n" -- not sure if I need to average elevation by species, or this is doing that. 
