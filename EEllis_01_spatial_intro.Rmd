...
```{r}
co_elevation_rast <- rast(co_elevation)
```

Now check the class:

```{r}
class(co_elevation_rast)
```

Now we can use `terra` functions, and re-run the `crop()` code we tried earlier:

```{r}
co_elevation_crop <- crop(co_elevation_rast, ext(poudre_hwy))
```
...
poudre_points_elev <- extract(co_elevation_crop, poudre_points_prj)
```
...
class(ca_elevation)
 ...
 LA_crop <- crop(ca_elevation, LA_county, mask = TRUE)

qtm(LA_county, fill = NULL) +
  qtm(LA_crop)
```
