...
# Check the CRS of the rivers object
crs_rivers <- st_crs(rivers)

# Check the CRS of the occurrences object (occ_prj)
crs_occurrences <- st_crs(occ_prj)

# Compare the CRS of the two objects
identical_crs <- identical(crs_rivers, crs_occurrences)

# Print the result
if (identical_crs) {
  cat("The CRS of rivers and occurrences objects are the same.\n")
} else {
  cat("The CRS of rivers and occurrences objects are different.\n")
}
...
species_avg_distance <- occ_larimer %>%
  group_by(Species) %>%
  summarize(avg_distance = mean(river_dist_m))

# plot
ggplot(species_avg_distance, aes(x = Species, y = avg_distance)) +
...
Which species is, on average, found closest to a river? The Wester Tiger Salamander, on average, is found closest to a river compared to Elk and Yellow-bellied marmots.
...
A: Political boundaries don't represent ecosystems very well, so a better method may be to use the EPA Ecorgeions boundaries/rasters instead of state/county boundaries/rasters.
...
```{r}
landcover_freq <- terra::freq(landcover)

# convert to data frame
landcover_df <- as.data.frame(landcover_freq)
...
```{r}
# join county_landcover & classes
county_landcover <- left_join(county_landcover, classes, by = "value")

# ggplt of counties filled by landcover (map)
ggplot() + 
  geom_sf(data = county_landcover, aes(fill = NLCD.Land.Cover.Class)) +
  labs(title = "Most Common Land Cover by County",
       fill = "Land Cover")

# realizing that i could have used qtm instead of ggplot
```
...
```{r}
# pull in elevation & occurence data
# side note: only works when I load in data from 01_spatial_intro
class(occ_prj)
class(co_elevation)

# match
st_crs(co_elevation) == st_crs(occ_prj)
```

```{r}
# transform the CRS of co_elevation
occ_prj_elevation <- st_transform(occ_prj, st_crs(co_elevation))

# match
st_crs(co_elevation) == st_crs(occ_prj_elevation)
```


```{r}
# extraction
co_occ_elevation <- project(co_elevation, crs(occ_prj_elevation))
extract(co_elevation_prj, occ_prj_elevation)

# mutate & unnest
occ_elevation <- occ_prj_elevation %>%
  mutate(common_elevation = extract(co_elevation_prj, occ_prj_elevation)) %>%
  unnest(common_elevation)

# avg
average_elevation <- occ_elevation %>%
  st_drop_geometry() %>% 
  group_by(Species) %>% 
  summarise(avg_elevation = mean(Elevation, na.rm = TRUE))

# was able to run this another time and get a table, but i'm running this again now to double check my answers and i'm getting an error... why is R like this :')

```
On average, marmots are more frequently found at higher elevations.

yellow-bellied marmot: 3301
elk - 2641
western tiger salamander: 2330