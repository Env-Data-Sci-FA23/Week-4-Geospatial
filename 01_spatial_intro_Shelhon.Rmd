...
```{r}
ct_elevation <- rast(ct_elevation)
```
...
```{r}
ct_counties_sf <- st_as_sf(ct_counties, coords = c("long", "lat"), crs = 4326)
```

```{r}
cte_points_prj <- st_transform(ct_counties_sf, st_crs(ct_elevation))
```

```{r}
ct_counties_rasy <- rast(ct_counties)
```

```{r}
ct_elevation_crop <- crop(ct_elevation, ext(ct_counties_sf))
```
...
```{r}
qtm(ct_elevation_crop) +
  qtm(ct_counties, fill = NULL)
...
```{r}
fairfield_ct <- ct_counties_sf %>%   
  filter(COUNTYFP == "001")
```

```{r}
fairfield_co_rast <- rast(fairfield_ct)
```

```{r}
ct_elevation_crop <- crop(ct_elevation, ext(fairfield_ct))
```
