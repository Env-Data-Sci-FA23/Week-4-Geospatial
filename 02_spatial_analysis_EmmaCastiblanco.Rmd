...
# Filter for Larimer County
larimer_county <- co_counties[co_counties$NAME == "Larimer", ]
...
species_avg_distance <- occ_larimer %>%
  ...
  group_by(Species) %>%
  summarise(avg_distance = mean(river_dist_m, na.rm = TRUE))

ggplot(species_avg_distance, aes(x = Species, y = avg_distance, fill=Species)) +
...
Which species is, on average, found closest to a river? ---\> Elk!
...
Now we can calculate what percentage of occurrences are within 100 m of a river for each species using `dplyr` operations. Which species is most often found within 100m of a river? --\> Elk
...
```{r}
ggplot(freq_landcover, aes(x = reorder(value, count), y = count)) +
  geom_bar(stat = "identity", fill = "darkgreen", color = "black") +
  coord_flip() +
  labs(title = "Landcover Frequency",
       x = "Landcover Type",
       y = "Frequency") +
  theme_minimal()
```
...
```{r}
setwd("~/Documents/R Studio/Environmental Data Science Applications/Week_4/Week_4_Geospatial")
```

```{r}
merged_data<- full_join(classes, county_landcover, by = "value") 

most_common_NLCD <- merged_data %>%
  filter(!is.na(NLCD.Land.Cover.Class)& NLCD.Land.Cover.Class != "") %>%
  group_by(NAME) %>%
   summarize(Most_Common_Class = names(which.max(table(NLCD.Land.Cover.Class))))

most_common_NLCD_counties<- right_join(most_common_NLCD,co_counties, by="NAME") %>%
  st_as_sf()

most_common_NLCD_counties

qtm(most_common_NLCD_counties, fill="Most_Common_Class")

```

### Exercise 6

Find the average elevation each species occurs at (for all Colorado occurrences). Which species is, on average, found at the highest elevations? --\> Yellow-bellied Marmot
...
st_crs(occ_prj)==st_crs(elevation)


elevation_prj <- project(elevation, crs(occ_prj))
extract(elevation, occ_prj)
...
occ_elevation_species <- occ_elevation %>%
   group_by(Species) %>%
   summarize(Avg_Elevation = mean(Elevation, na.rm = TRUE))

highest_avg_elevation_species <- occ_elevation_species %>%
  filter(Avg_Elevation == max(Avg_Elevation))
