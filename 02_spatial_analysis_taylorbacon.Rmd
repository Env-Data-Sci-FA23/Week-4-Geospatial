...
```{r}
mean_river_dist <- occ_larimer %>%
  mutate(as.numeric(river_dist_m))%>%
  group_by(Species)%>%
  summarise(mean_river_dist_m = as.numeric(mean(river_dist_m)))

mean_river_dist%>%
  ggplot()+
  geom_col(aes(x=Species, y=mean_river_dist_m))+
  labs(y = "Mean Distance from River (m)")
```
...
occ_rivers %>% 
  ...
  summarise(total_occ = n(), 
            ...
            percent_river = (total_river/total_occ)*100) 
```
...
By just looking at data within Larimer county (i.e. a political boundary) we may not be looking at the most representative sample for each species, since the landcover, elevation, etc. of just Larimer county will likely skew the data compared to looking at the state as a whole.
...
The yellow-bellied marmot is found at the highest average elevation.
