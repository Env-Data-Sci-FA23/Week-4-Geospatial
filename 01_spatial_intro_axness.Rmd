...
It is a data frame labeled sf for simple feature (more below).
...
1.  **Explore the use of `extract` from the `terra` package by running `?terra::extract`. (Note we need to specify `terra::` because 'extract' is a function name in multiple packages we may have loaded in our session).**
    ...
    **How would you extract the elevation at each of the three points in `poudre_points_prj` ? (2 pts)**
    ...
    poudre_points <- data.frame(
      ...
      lon = c(-105.3563, -105.5816, -105.8556),
      lat = c(40.68752, 40.69687, 40.5796)
    )

    # Convert to a SpatialPoints object
    poudre_points_sp <- vect(poudre_points, geom=c("lon", "lat"), crs="+proj=longlat")

    # Assuming you have a raster object `elevation_raster` that contains elevation data
    # elevation_raster <- ...

    # Extract elevation data
    elevation <- terra::extract(elevation_raster, poudre_points_sp)

    # Print the extracted elevation data
    print(elevation)
...
# Download and plot county boundaries for New Mexico
nm_counties <- counties(state = "NM", class = "sf")
plot(st_geometry(nm_counties))
...
# Get elevation data for New Mexico
nm_elevation <- get_elev_raster(nm_counties, z = 7)
plot(nm_elevation)
...
```{r}
# Filter Santa Fe county
santa_fe_county <- nm_counties %>% 
  dplyr::filter(NAME == "Santa Fe")

# Crop elevation data to Santa Fe county
santa_fe_elevation <- crop(nm_elevation, extent(st_bbox(santa_fe_county)))

# Plot
plot(santa_fe_elevation)
