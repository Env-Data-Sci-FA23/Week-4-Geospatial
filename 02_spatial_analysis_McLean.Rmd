...
```{r}
occ_larimer <- st_filter(occ_prj, larimer,.predicate = st_intersects)
```


```{r}
qtm(occ_larimer)+
  qtm(larimer, fill = NULL)
```
...
```{r}
occ_larimer1 <- occ_larimer %>%
  mutate(river_dist = substr(river_dist_m, 1, 9))
```



```{r}
occ_larimer2 <- occ_larimer1 %>% 
  mutate(river_dist = as.numeric(river_dist))
```

```{r}
glimpse(occ_larimer2)
```

<!-- Now to do it right for fun -->
```{r}
occ_larimer_test <- occ_larimer %>% 
  mutate(river_dist = as.numeric(river_dist_m))
```

```{r}
avg_dist <- occ_larimer2 %>% 
  group_by(Species) %>% 
  summarize(avg_river_dist = mean(river_dist)) %>% 
  arrange(desc(avg_river_dist))
...
Animals don't adhere to political boundaries and can move in and out of them at will. This is a cross section of a certain time but might not accurately capture the true numbers
...
```{r}
ggplot()+
  geom_col(data = landcover_freq, mapping = aes(x = reorder(value, -count), y = count))+
...
```{r}
ggplot()+
  geom_col(data = landcover_freq, mapping = aes(x = count, y = reorder(value, -count)))
```
...
terra::extract(landcover_prj, occ_prj)
```
...
occ_landcover <- occ_prj %>%
  mutate(common_landcover = terra::extract(landcover_prj, occ_prj)) %>%
...
county_landcover <- 
  counties %>%
  mutate(landcover = terra::extract(landcover_prj, counties, fun = "modal")) %>%
  unnest(landcover) %>%
...
terra::extract(elevation_prj, occ_prj)
```


```{r}
occ_elev <- occ_prj %>%
  mutate(common_elevation = terra::extract(elevation_prj, occ_prj)) %>%
  unnest(common_elevation)
```


```{r}
avg_occ_elev <- occ_elev %>% 
  st_drop_geometry() %>%
  group_by(Species) %>% 
...
Yellow-Bellied Marmots occur at the highest average elevation
