...
larimer_county <- st_transform(larimer_county, st_crs(occ_prj))
...
st_crs(occ_larimer)
st_crs(rivers)
#transform occ_larimer to match CRS of rivers 
occ_larimer <- st_transform(occ_larimer, crs = st_crs(rivers))
...
```{r}
occ_larimer_2 <- occ_larimer %>%
  group_by(Species) %>%
  mutate(river_dist = as.numeric(river_dist_m)) %>%
  summarise(avg_river_distance = mean(river_dist))

ggplot(data = occ_larimer_2) +
  geom_col(aes(x = Species, y = avg_river_distance, fill = Species), position = "stack") +
  labs(x = "Species", y = "Avg distance to nearest river (m)") +
  scale_fill_manual(values = c("Elk" = "orange", "Western Tiger Salamander" = "forestgreen", "Yellow-bellied Marmot" = "blue")) +
...
Answer: Elk is most often found within 100m of a river
...
Answer: I would be cautious about this because animals do not follow boundary lines or buffer lines. The population of all 3 species are always changing within Larimer County, so it is hard to say for a fact that these statistics are accurate.
...
```{r}
landcover_freq <- landcover_freq[order(-landcover_freq$count), ]

landcover_freq %>%
  ggplot(aes(x = reorder(value, -count), y = count)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(x = "Landcover type", y = "Frequency") +
  coord_flip()
```
...
```{r}
class_landcover <- left_join(county_landcover, classes)

#mapping
qtm(class_landcover, fill = "NLCD.Land.Cover.Class")

#Can also run it as a ggplot map
ggplot(data = class_landcover, aes(fill = NLCD.Land.Cover.Class)) +
  geom_sf() +
  #scale_fill_continuous(values = county_landcover$value) +
  labs(title = "NLCD Land Cover by County") +
  theme_minimal()
```
...
occ_prj_2 <- st_transform(occ_prj, st_crs(co_elevation))

# Now check that they match
st_crs(co_elevation) == st_crs(occ_prj_2)

#combine elevation into a prj
co_elevation_prj <- project(co_elevation, crs(occ_prj_2))
extract(co_elevation_prj, occ_prj_2)

#nesting elevation into a single column in occ_
occ_elevation_prj <- occ_prj_2 %>%
  mutate(common_elevation= extract(co_elevation_prj, occ_prj_2)) %>%
  unnest(common_elevation) %>% 
  #rename elevation order
  rename(common_elevation = "filebf961e8a7f27")

#finding the average elevation per species
avg_elevation <- occ_elevation_prj %>%
  st_drop_geometry() %>% # this converts the data back to a dataframe, required for some tidyverse operations
  group_by(Species) %>% 
  summarise(average = mean(common_elevation))
```

Answer: The average elevation each species is found at is as follows:

Elk - 2641.747 m

Western Tiger Salamander - 2329.969 m

Yellow-bellied Marmot -- 3301.369 m

Which means that the marmot species is found at the highest elevation
