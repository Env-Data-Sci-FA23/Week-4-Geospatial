...
There are a ton of ways to customize these maps (more details on this in the spatial viz lesson!). For example, `counties` has an 'ALAND' variable, which represents the total land area of each county. To color by that variable we would use:
...
Import elevation for your state (using your new counties object as the extent/bounding box):
...
Choose a single county within your state county object, and crop your elevation layer to the extent of that county (*note:* use `filter()` to create an object of just your selected county that you want to crop to).
