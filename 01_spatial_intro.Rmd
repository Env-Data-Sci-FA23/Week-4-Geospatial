...
We can use the `terra` package to work with raster data. For example, we only want to see elevation along the Poudre highway. We can use `crop` to crop the raster to the extent of our `poudre_hwy` shapefile using the `ext()` function to get the extent (i.e., bounding box) of our `poudre_hwy` object.
...
`terra` using objects of a new class called `SpatRaster`. Converting a `RasterLayer` to a `SpatRaster` is quick using the `rast()` function.
