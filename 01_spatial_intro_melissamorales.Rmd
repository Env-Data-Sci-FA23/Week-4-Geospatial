...
# elevation values at the three points
elevation_values <- terra::extract(elevation, poudre_points_prj)
```
...
```{r}
#ny_elevation_crop <- crop(ny_elevation, ext(ny_counties))
``` 
...
#   tm_borders(lwd = 0.5) +
#   tm_shape(ny_elevation_crop) +
#   tm_raster(style = "cont") + #tried using `fill =` here and I couldn't get this to work to remove the fill color
...
```{r}
# crop
elevation_kings_county <- crop(ny_elevation, kings_county, mask = TRUE)
```

```{r}
# interactive map
qtm(elevation_kings_county) + 
  qtm(kings_county, fill = NULL)
```
