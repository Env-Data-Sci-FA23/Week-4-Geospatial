...
```{r}
larimer <- counties %>% filter(NAME == "Larimer")
...
occ_larimer %>% 
  ...
  group_by(Species) %>% 
  summarize(mean_distance = mean(river_dist_m)) %>% 
  ggplot(aes(x = Species, y = mean_distance)) +
  geom_col()
...
Now we want to know what percentage of forest is found in our species' habitats. In this case we are going to define 'habitats' as a 1 km region around each species occurrence.

So first, let's buffer our points, using the occurrences for just Larimer county (`occ_prj`) for faster computing and using `5000` as our distance since length units of our CRS is in meters. *Note:* this may take a few seconds to run

```{r}
occ_buffer <- st_buffer(occ_larimer, 2000)
```

```{r}
qtm(occ_buffer, fill = NULL) +
  qtm(landcover)
```

`occ_buffer` is now a polygon layer, that we can use as our 'zones' to calculate percent forest cover within.

But first! We are about to do some analysis, we have to check if the CRS of our two objects match. With rasters (and vectors) you can use `crs()` to return the CRS of that object. So, we can check if they match like this:

```{r}
crs(occ_buffer) == crs(forest)
```

Oh no, they are not the same! It technically doesn't matter which object we transform the CRS of, but to learn how to transform rasters let's transform our forest layer to the CRS of the occurrences. In terra, we can transform with the `project()` function. Let's create a new object for this transformed raster layer:

```{r}
forest_prj <- project(forest, crs(occ_buffer))
```

Double check that they match now:

```{r}
crs(forest_prj) == crs(occ_buffer)
```

Yay! Now we can continue with the analysis. There are a couple ways to perform zonal statistics

```{r}
zonal(forest_prj, vect(occ_buffer), fun = sum, na.rm = TRUE)
```
