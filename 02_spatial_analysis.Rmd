...
occ_landcover <- occ_prj %>%
  mutate(common_landcover = extract(landcover_prj, .)) %>%
...
Use `extract()` to find the most common landcover type in each Colorado county.

*Hint*: For polygon operations, explore which mathematical function you can use to return the most common element...Using `?terra::extract` might help.
...
```{r}
county_lc_joined <- inner_join(county_landcover, classes, by = "value")

qtm(county_lc_joined, fill = "NLCD.Land.Cover.Class")
```
