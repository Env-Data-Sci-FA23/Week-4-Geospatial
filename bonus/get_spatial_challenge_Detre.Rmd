...
output_species <- vector("list", length = length(species))

for(i in 1:length(output_species)){
  
  output_species[[i]] <- occ_data(
...
multi_species <- dplyr::bind_rows(output_species)
  
  # add species name column as ID to use later
  multi_species$ID <- common_name
  
  #clean by removing duplicate occurrences
 multi_species <-
    multi_species %>% distinct(decimalLatitude, decimalLongitude, .keep_all = TRUE) %>%
