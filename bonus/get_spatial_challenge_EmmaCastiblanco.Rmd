...
```{r}
species_list <- c("Cervus canadensis", "Marmota flaviventris", "Ambystoma mavortium")
common_name_list <- c("Elk", "Yellow-bellied Marmot", "Western Tiger Salamander")

data_list <- list()

for (i in seq_along(species_list)) {
  current_species <- species_list[i]
  current_common_name <- common_name_list[i]


occ <-
    occ_data(
      scientificName = current_species,
...
```{r}
occ <- st_as_sf(combined_data, coords = c("decimalLongitude", "decimalLatitude"), crs = 4326)
```
...
occurrences_list <- purrr::map(1:length(species_list), function(i) {
  current_species <- species_list[i]
  current_common_name <- common_name_list[i]
...
```{r}
Larimer<- SCAN_SNOTEL_metadata %>%
  filter(County=="Larimer")

snotel_sites <- st_as_sf(Larimer, coords = c("Longitude", "Latitude"), crs = 4326)
```

How many SNOTEL sites are located in Colorado? --\> 8
...
fetch_snotel_data <- function(x) {
  fetchSCAN(site.code = x, year = 2020:2022, timeseries = "Daily") %>%
    bind_rows() %>%
    as_tibble() %>%
    filter(sensor.id == "SNWD.I") %>% 
    select(-starts_with("Name"), -pedlabsampnum)
}


sites_list <- unique(snotel_sites$Site)
data_list <- purrr::map(sites_list, ~fetch_snotel_data(x = .x))



combined_sites<- bind_rows(data_list) 
snow_depth <- left_join(snotel_sites, combined_sites, by= "Site")

final_data_sf <- st_as_sf(snow_depth, crs = 4326)
