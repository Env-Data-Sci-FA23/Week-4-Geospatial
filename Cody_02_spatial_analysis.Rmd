...
avg_river_dist <- occ_larimer%>%
  mutate(river_dist_m = as.numeric(river_dist_m))%>%
  group_by(Species)%>%
  summarise(avg_dist = mean(river_dist_m))
```
...
Political boundaries have no meaning to ecosystems and species behavior. You can draw conclusions about these species' behavior in Larimer County but not about Colorado or the species as a whole.
...
```{r}
landcover_freq <- landcover_freq%>%
  arrange(desc(count))
ggplot(data=landcover_freq)+
...
```{r}
elevation_prj <- project(elevation, csr(occ_prj))
