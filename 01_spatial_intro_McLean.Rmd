...
```{r}
fl_elevation <- get_elev_raster(fl_counties, z = 7)
```
...
```{r}
hills_county <- fl_counties %>% 
  filter(NAME == "Hillsborough")
```


```{r}
fl_elevation <- rast(fl_elevation)
```
...
```{r}
fl_elevation_crop <- crop(fl_elevation, hills_county, mask = TRUE)
```
...
```{r}
qtm(hills_county, fill = NULL)+
  qtm(fl_elevation_crop)
```
