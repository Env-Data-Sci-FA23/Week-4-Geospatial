...
# Transform the CRS of occ_sp to match that of rivers
occ_prj <- st_transform(occ_sp, st_crs(rivers))

# Check if the CRS match now
crs_match <- st_crs(occ_prj) == st_crs(rivers)
...
# Now, filter occ_prj to only include points within Larimer County
...
# Add the index of the nearest river feature to occ_larimer
occ_larimer$nearest_river <- st_nearest_feature(occ_larimer, rivers)

# Calculate the distance to the nearest river feature for each point
# and add it as a new column 'river_dist_m' to the occ_larimer object
occ_larimer$river_dist_m <- st_distance(occ_larimer, 
                                        rivers[occ_larimer$nearest_river, ], 
                                        by_element = TRUE)

# Convert the units object to numeric (meters)
occ_larimer$river_dist_m <- as.numeric(occ_larimer$river_dist_m)
...
species_avg_dist <- occ_larimer %>%
  group_by(Species) %>%
  summarize(average_distance = mean(river_dist_m, na.rm = TRUE)) %>%
  ungroup()  # Ungroup the data frame after summarize

# create the bar plot
ggplot(species_avg_dist, aes(x = Species, y = average_distance, fill = Species)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(x = "Species", y = "Average Distance to Nearest River (m)", title = "Average River Distance by Species") +
  coord_flip()  # Flip the coordinates for better readability of species names
...
Elk have the overall smallest range from rivers at just over 1000m distance to nearest river.
...
# Create a new logical column in occ_larimer indicating TRUE if the occurrence is within 100m of a river
occ_larimer <- occ_larimer %>%
  mutate(river_100m = if_else(lengths(river_intersections) > 0, TRUE, FALSE))
...
# the first few rows to confirm the new column
head(occ_larimer)
...
```{r}
# Calculate the percentage of occurrences within 100m of a river for each species
species_percentage_near_river <- occ_larimer %>%
  group_by(Species) %>%
  summarize(percentage_within_100m = mean(river_100m) * 100) %>%
  ungroup() %>%
  arrange(desc(percentage_within_100m))  # Arrange by descending order of the percentage

# Display the species and their percentages
print(species_percentage_near_river)

# Identify the species most often found within 100m of a river
most_common_species_near_river <- species_percentage_near_river %>%
  slice(1)  # Select the top row which has the highest percentage
...
I would be cautious in using this data to make an overall interpretation because as mentioned above animals are not limited by political boundaries the same goes for landscapes. Rivers generally don't make up division lines and not all rivers are of the same size.
...
```{r}
# Calculate the frequency of each landcover type
landcover_freq <- freq(landcover)

# Convert to a data frame
landcover_freq_df <- as.data.frame(landcover_freq)

# create the bar chart
ggplot(landcover_freq_df, aes(x = reorder(value, -count), y = count)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  theme_minimal() +
  labs(x = "Landcover Class", y = "Frequency", title = "Landcover Class Frequency") +
...
```{r}
#assign landcover to a new object name so we can manipulate it while keeping the original
forest <- landcover
...
occ_landcover <- occ_prj %>%
  ...
  #rename the land cover column which is now called "NLCD Land Cover Class"
...
```{r}
#names(county_landcover)
#names(classes)

#Verify column names and types in both data frames
# Check column names and types in county_landcover
str(county_landcover)

# Check column names and types in classes
str(classes)

```

```{r}
# Convert 'value' in county_landcover to numeric if it's not already
county_landcover$value <- as.numeric(as.character(county_landcover$value))

# Convert 'value' in classes to numeric if it's not already
classes$value <- as.numeric(as.character(classes$value))

# Now perform the join
joined_data <- left_join(county_landcover, classes, by = "value")

```

```{r}

cleaned_county_landcover <- joined_data %>%
  select(STATEFP:AWATER, geometry, NLCD.Land.Cover.Class.y) %>%
  rename(value = NLCD.Land.Cover.Class.y)

```

```{r}

# Assuming 'county_landcover_joined' now has a geometry column for plotting
# Plot the map with ggplot2
ggplot(data = cleaned_county_landcover) +
  geom_sf(aes(fill = value)) +  # Fill the counties by the most common land cover class
  scale_fill_viridis_d(option = "D") +  # Use a discrete color scale from the viridis package
  labs(title = "Most Common NLCD Land Cover Class by County",
       fill = "Land Cover Class") +  # Label the legend
...
The species that occurs at the highest elevations for all of Colorado on average are ELK.
...
# Check and match CRS
crs_occ_prj <- st_crs(occ_prj)
crs_elevation <- crs(elevation)

if (crs_occ_prj != crs_elevation) {
  occ_prj <- st_transform(occ_prj, crs_elevation)
}

# Extract elevation data for each occurrence
occ_elevation <- terra::extract(elevation, occ_prj)

# Combine the elevation data with the species occurrence data
occ_prj$elevation <- occ_elevation

# Calculate average elevation per species
average_elevations <- occ_prj %>%
  group_by(Species) %>%  
  summarize(avg_elevation = mean(elevation, na.rm = TRUE))

#Identify species found at the highest elevations
highest_elevation_species <- average_elevations %>%
  arrange(desc(avg_elevation)) %>%
  head(1)

view (highest_elevation_species)
```
