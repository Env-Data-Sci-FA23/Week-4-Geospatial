...
species_avg_dist <- occ_larimer %>% 
  group_by(Species) %>% 
  summarise(average_dist = mean(river_dist_m)) %>% 
  mutate(average_dist = as.numeric(average_dist))


```

```{r}
ggplot(data = species_avg_dist, aes( x = Species, y= average_dist, fill= Species)) +
  geom_col()
```

Which species is, on average, found closest to a river?

On average, the Elk is found closest to a river
...
Elk are most often found within 100m of a river, with the percentage being 16.3%
...
Species occurrence do not conform to political boundaries, so we may not be analyzing the full picture.
...
```{r}
frequency_lc <- freq(landcover)

ggplot(data = frequency_lc) +
  geom_col(aes(x= reorder(value, -count), y=count, fill= value)) +
  theme_bw() +
 coord_flip()
```
...
```{r}
joined_landcover <- left_join(county_landcover, classes)

qtm(joined_landcover, fill="NLCD.Land.Cover.Class")
```
...
elevation_occ <- occ_prj %>% 
  mutate(elevation = extract(elevation, occ_prj)) %>% 
  unnest(elevation) 
...
The Yellow-bellied Marmot is on average, found at the highest elevations (3302 m)
