...
```{r}
river_dist <- occ_larimer %>% 
  mutate(river_dist = as.numeric(river_dist_m))

species_average_distance <- river_dist %>%
  group_by(Species) %>%
  summarise(avg_distance = mean(river_dist, na.rm = TRUE))

species_average_distance %>%
  ggplot() +
  geom_bar(aes(x = Species, y = avg_distance), stat = "identity") +
  labs(x = "Species", y = "Average Distance to River (meters)")
```
...
```{r}
landcover_frequency <- freq(landcover) 
```
...
```{r}
landcover_frequency <- landcover_frequency %>%
  arrange(desc(count))
...
```{r}
joined_data <- inner_join(county_landcover, classes, by = "value")

joined_data <- joined_data %>%
  rename(Landcover = NLCD.Land.Cover.Class)

most_common_landcover <- joined_data %>%
  group_by(NAME) %>%
  summarise(mode_landcover = names(which.max(table(Landcover))))

ggplot(most_common_landcover) +
  geom_sf(aes(fill = mode_landcover)) +
  labs(title = "Most Common NLCD Land Cover by County")
...
occ_with_elevation <- cbind(occ_prj, elevation = elevation_values) #%>%
 # st_as_sf(coords = c("decimalLongitude", "decimalLatitude"), crs = 4326)

avg_elevation_by_species <- occ_with_elevation %>%
  st_drop_geometry() %>%
  group_by(Species) %>%
  summarise(avg_elevation = mean(elevation.Elevation, na.rm = TRUE))
```
