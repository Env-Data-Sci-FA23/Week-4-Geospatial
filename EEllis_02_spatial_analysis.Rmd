...
species_dist <- occ_larimer %>% group_by(Species) %>% summarise(mean = mean(river_dist_m))

river_dist_plot <- ggplot(species_dist, aes(x = Species, y = mean)) +
  geom_col() +
  labs(
    title = "Species occurence - average distance to river",
    x = "Species",
    y = "Occurence distance from river (m)"
  ) +
  theme_minimal() + 
...
This analysis is just for teaching purposes, why would you be cautious about these results for answering real research questions? Think about how we filtered everything to a political boundary, what's wrong with this method?
 ...
 # I would be cautious about using these results for real research purposes because animals do not care about county boundaries. The county boundary is an arbitrary cutoff. I would use a water shed or mountain range cut off, something that might confine animals to a geographic area. 
...
```{r}
land_cover_freq <- freq(landcover)
land_cover_freq
```
...
```{r}
land_cover_freq$value <- reorder(land_cover_freq$value, -land_cover_freq$count)

land_cover <- ggplot(land_cover_freq, aes(x = value, y = count)) +
  geom_col() +
  labs(
    title = "Colorado landcover",
    x = "Landcover type",
    y = "Number of pixels"
  ) +
  theme_minimal() + 
...
```{r}
joined_data <- inner_join(x = county_landcover, y = classes, by = "value")
view(joined_data)
...
```{r}
st_crs(elevation) == st_crs

species_elevation <- extract(elevation, occ_prj)

occ_elevation <- occ_prj %>% 
  mutate(spec_elev = extract(elevation, occ_prj)) %>%
  unnest(spec_elev) %>%
  rename(spec_elev = "Elevation")

occ_elev_mean <- occ_elevation %>% 
  st_drop_geometry() %>% group_by(Species) %>% 
  summarise(mean = mean(spec_elev, na.rm = TRUE))

occ_elev_mean #Yellow-belied marmots are found at the highest elevation, on average 
```
